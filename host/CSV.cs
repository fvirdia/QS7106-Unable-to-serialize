...
namespace cs
{
    // To generate the right templates here, check the first row returned by
    // Dictionary<string, string> csv = sim.ToCSV();
    // Console.Write(csv["WidthCounter"]);
    // Console.Write(csv["DepthCounter"]);
    // Console.Write(csv["PrimitiveOperationsCounter"]);
    [DelimitedRecord("\t")]
    [IgnoreFirst(1)]
    ...
    public class QDecimalConverter : ConverterBase
    {
        public override object StringToField(string from)
        {
            ...
            {
                // Clumsy bodge: Scientific notation breaks the parser
                // Here it tries to detect and repair such strings
                if (from.Contains('E')){
                    return Decimal.Parse(from, System.Globalization.NumberStyles.AllowExponent | System.Globalization.NumberStyles.AllowDecimalPoint);
                }
